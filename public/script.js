    ...
    fetch('/api/getDiscordUser', {
        ...
        .then((data) => {
            ...
            if (data.error) {
                ...
                errorCard.innerHTML = `<p>Error: ${data.error}</p>`;
        ...
        .catch((error) => {
            ...
            const errorCard = document.createElement('div');
            errorCard.className = 'card';
            errorCard.innerHTML = '<p>An error occurred. Please try again later.</p>';
            document.getElementById('cardsContainer').innerHTML = ''; // Clear existing cards
